...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        try:
            ...
            self.datetime_edit.setFixedSize(195, 30)
            ...
            # News button
            news_button = QAction(QIcon("icons/news_icon.png"), 'News', self)
            news_button.setObjectName("tool")
            news_button.triggered.connect(self.open_news)
            toolbar.addAction(news_button)
    ...
    def open_news():
        try:
            subprocess.Popen(['python', 'news_gui.py'])

        except Exception as e:
            print(f"An error occurred while opening the news code: {e}")
